...
public abstract class SearchResultAdapter extends RecyclerView.Adapter<SearchResultAdapter.MyViewHolder> {
    Context c;
    List<GetAllLatLongPoints.DataBean> dataBeans = new ArrayList<>();

    public SearchResultAdapter(Context c, List<GetAllLatLongPoints.DataBean> data) {
        this.c = c;
        this.dataBeans = data;
    }
    @NonNull
    @Override
    public SearchResultAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
        View v = LayoutInflater.from(c).inflate(R.layout.custom_fav_waypoint, viewGroup, false);
        return new MyViewHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull SearchResultAdapter.MyViewHolder myViewHolder, final int i) {
